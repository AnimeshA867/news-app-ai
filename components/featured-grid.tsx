    ...
    return (
      <div className="rounded-lg border p-8 text-center">
        ...
        <p className="mt-2 text-muted-foreground">
          Check back soon for featured stories.
        </p>
      </div>
    );
  }
        ...
        >
          <Link href={`/article/${mainArticle.slug}`} className="group block">
            ...
            <div className="absolute bottom-0 left-0 right-0 p-4 md:p-6">
              ...
              <p className="mb-4 hidden text-white/90 md:block">
                {mainArticle.excerpt}
              </p>
              <div className="flex items-center gap-4 text-sm text-white/80">
                <span>{mainArticle.author?.name}</span>
                <span className="flex items-center gap-1">
                  <Clock className="h-3 w-3" /> {mainArticle.readTime || 5} min
                  read
                </span>
              </div>
            </div>
          ...
          >
            <Link href={`/article/${article.slug}`} className="group block">
              ...
              <div className="p-3">
                ...
                <h3 className="mb-1 font-bold transition-colors group-hover:text-primary">
                  {article.title}
                </h3>
